...
def copy_arene(arene:dict)->dict:
    ...
    nb_ligne = matrice.get_nb_lignes(arene["matrice"])

    arene_copy = matrice.Matrice(nb_ligne,nb_col)
    current_arena_val = arene["matrice"]["valeurs"]
    #for pos,valeur in current_arena_val.items():
    #    arene_copy["valeurs"][pos] = valeur

    snake_copy = []
    for snake in arene["serpents"]:
        snake_copy.append(serpent.copy_serpent(snake))

    return {"matrice":arene_copy,"nb_joueurs":len(snake_copy),"serpents":snake_copy}