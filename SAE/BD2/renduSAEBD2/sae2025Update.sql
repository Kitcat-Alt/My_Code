...
with CA2024 as (select sum(qte*prixvente) as total
    from DETAILCOMMANDE natural join COMMANDE 
    where YEAR(datecom) = 2025)
select nomclass as Theme, ROUND((sum(qte*prixvente)/Total)*100) as total
...
where MONTH(datecom) = 02 and YEAR(datecom) = 2020 order by idmag, numcom, numlig;
